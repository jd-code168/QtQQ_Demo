	...
	,m_channel(new QWebChannel(this))
{
	...
	if (isGroupTalk) {
		QModelIndex employeeModeIndex, pictureModelIndex;
		int rows = queryEmpolyeeModel.rowCount();
		for (int i = 0; i < rows; i++) {
			employeeModeIndex = queryEmpolyeeModel.index(i, 0);
			pictureModelIndex = queryEmpolyeeModel.index(i, 1);

			strEmployeeID = queryEmpolyeeModel.data(employeeModeIndex).toString();
	...
	for (int i = 0; i < list.count(); i++) {
		...
		if (node.isElement()) {
			...
			if (element.tagName() == "img") {
				...
				attributList << "img" << element.attribute("src");  //attribute(获取属性)，获取src的属性路径
				attribute << attributList;
			}

			if (element.tagName() == "span") {

				QStringList attributList;
				attributList << "text" << element.text();
				attribute << attributList;
			}
