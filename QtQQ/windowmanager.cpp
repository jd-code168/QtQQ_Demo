...
void WindowManager::addNewTalkWindow(const QString & uid/*, GroupType groupType, const QString & strPeople*/)
{
	...
	if (!widget) {
		...
		indexDepIndex = sqlDepModel.index(0, 0); //模型索引0行0列
		signIndex = sqlDepModel.index(0, 1); //模型索引0行1列
		...
		/*switch (groupType) {
		case COMPANY: {
			talkwindow->setWindowName(QStringLiteral("悦分享越拥有"));
		...
		case MARKETGROUP: {
			talkwindow->setWindowName(QStringLiteral("市场部群"));
			talkwindowItem->setMsgLabelContent(QStringLiteral("市场群"));
			break;
		}
		case DEVELOPMENTGROUP: {
			talkwindow->setWindowName(QStringLiteral("研发部群"));
