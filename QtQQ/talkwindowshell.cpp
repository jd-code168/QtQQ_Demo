...
void TalkWindowShell::initControl()
{
	...
	setWindowTitle(QString::fromLocal8Bit("Demo-聊天窗口"));
	...
	ui.splitter->setSizes(leftWidgetSize); //分列器设置尺寸
...
void TalkWindowShell::handleReceivedMsg(int senderEmployeeID, int msgType, QString strMsg)
{
	...
	if (msgType == 1) {	//文本信息
		...
		for (int i = 0; i < emotionNum; i++) {
			msgTextEdit.addEmtionUrl(strMsg.mid(i * 3, emotionWidth).toInt());
		}
	}
...
/*****************************************************************************************************************************
	...
	（注意：当群聊标志为1时，则数据包没有收信息员工QQ号，而是收信息群号
			当群聊标志为0时，则数据包没有收信息群QQ号，而是收信息员工QQ号）

			群聊文本信息如：110001200100005Hello 表示QQ100001向群2001发送文本信息，长度是5，数据为Hello
			单聊图片信息如：0100011000201images060		 表示QQ100001向QQ10002发送1个表情60.png
			群聊文件信息如：11000052000210bytestest.txtdata_beginhelloword
												 表示QQ10005向群2000发送文件信息，文件时test.txt,文件内容长度10，内容helloword
			
			群聊文件信息解析：1 10001 2001 00005 Hello
...
void TalkWindowShell::processPendingData()
{
	...
	while (m_udpReceiver->hasPendingDatagrams()) {
		...
		QString strWindowID;	//聊天窗口ID，群聊这是群号，单聊则是员工QQ号
